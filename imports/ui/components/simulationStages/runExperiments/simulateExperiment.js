...
//set up svg inside div
    var svg = d3.select("#boardingProcess-canvas")
        .append("svg")
        .attr("width", "100%")
        //.attr("visibility","hidden") // ----------------- HIDE SVG ----------------
        .attr("height", "100%")
        .call(zoom)
        .append("g")
    ...
    function update() {
        ...
        //.data(data.filter(function(d) { return ( (parseInt(d.visible) === 1) && (parseInt(d.settled) === 0) && (parseInt(d.settling) === 0) ); }))
            .data(data.filter(function (d) {
                return ((parseInt(d.visible) === 1) && (parseInt(d.settled) === 0));
            }))
            .attr("cx", function (d) {
                return (d.x * scale) + parseInt(circleGroup.attr("x"));
            })
            .attr("cy", function (d) {
                return (-d.y * scale) + parseInt(circleGroup.attr("y"));
            })
            .attr("r", scale / 2)
            .attr("wait_current", function (d) {
                return d.wait_current;
            })
            .style("fill", function (d) {
                return d.settling === 1 ? '#6bff80' : '#B8DEE6'
            });
        ...
        var circle2 = planeGroup.selectAll("circle")
            ...
            .attr("serialNo", function (d) {
                return d.serialNo;
            })
            .style("fill", function (d) {
                return d.conflict === 1 ? '#ff5456' : '#E0995E'
            })
            .attr("cx", function (d) {
                return (d.x * scale) + parseInt(circleGroup.attr("x"));
            })
        ...
        var text = circleGroup.selectAll("text")
            .data(data.filter(function (d) {
                return ((parseInt(d.visible) === 1) && (parseInt(d.visible_text) === 1));
            }))
            .attr("class", "centre-text")
            .attr("x", function (d) {
                return (d.x * scale) + parseInt(circleGroup.attr("x"));
...
//actions every game tick: data
    function countDown() {
        ...
        if ((data.filter(function (d) {
            return ((parseInt(d.visible) === 0) || (parseInt(d.settled) === 0));
        })).length === 0) {
